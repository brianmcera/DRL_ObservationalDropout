...
def render_gym(st_object, env, episode, width=300):
    #img = env.render(mode='rgb_array')
    #caption = "Episode " + str(episode)
    st_object.image(env.render(mode='rgb_array'), width=width)#img)#, caption = caption, width = width)
    return 
...
reward_pd = pd.DataFrame([[0.0, 0.0, 0.0]], columns=('Batch Reward Means','Batch Reward Variance','Batch Maximum Reward'))
...
st_dict = {'render_obj':st_object, 'render_fcn':render_gym, 'progress_bar':st_bar, 'ac_prob_plot':ac_prob_plot, 'value':value_plot, 
        'action_dist':action_dist, 'advantage_dist':advantage_dist, 'entropy_plot':entropy_plot}


with tf.Graph().as_default():
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    log_dir = 'logs/tensorboard/' + current_time
    tensorboard_callback = None #tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1) # currently not working
    ...
    with open('../data/training_logs/' + start_time + '_' + \
        ...
        for _ in range(args.total_steps // args.batch_size + 1):
            ...
            new_rows = pd.DataFrame([[rewards_record['means'][-1], rewards_record['stds'][-1], rewards_record['max'][-1]]], 
                    columns=('Batch Reward Means','Batch Reward Variance','Batch Maximum Reward'))
            st_plot.add_rows(new_rows)
            #std_plot.add_rows(rewards_record['stds'][-1])
            #max_plot.add_rows(rewards_record['max'][-1])
