...
class Agent(Agent_Wrapper):
    ...
    def train(self, env, batch_sz=5000, updates=1, show_visual=True, random_action=False, show_first=False, tb_callback=None, sl_fcn=None, sl_obj=None, v_recorder=None):
...
def main():
    ...
    video = video_recorder.VideoRecorder(env, base_path='video_test_3')
    video.output_frames_per_sec = 60
    ...
    with tf.Graph().as_default():
        ...
        rewards_history, _ = agent.train(
                ...
                tb_callback=None, v_recorder=video)
        ...
        with open('../logs/' + start_time + '_' + model.model_name + '.csv', mode='w') as csv_file:
            ...
            for _ in range(args.total_steps // args.batch_size + 1):
                ...
                rewards_history, policy_entropy = agent.train(env, 
                        ...
                        show_visual=args.visual, 
                        show_first=args.show_first,
                        tb_callback=tensorboard_callback, v_recorder=video)
