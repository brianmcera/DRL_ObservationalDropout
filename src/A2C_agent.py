...
class A2CAgent:
    def __init__(self, model, lr=1e-3, gamma=0.80, value_c=1, entropy_c=1e-4):
        self.model = model
    ...
    def train(self, env, batch_sz=2000, updates=1, show_visual=True, random_action=False):
        ...
        for update in range(updates):
            for step in range(batch_sz):
                ...
                if(show_visual):
                    env.render()
                if(random_action):
                    action, values[step] = self.model.action_value(
                            next_obs[None,None,:])
                ...
                if dones[step]:
                    _, next_value = self.model.action_value(
                            tf.expand_dims(next_obs[None, :],0))
            ...
            returns, advs = self._returns_advantages(
                    rewards, dones, values, tf.squeeze(next_value,0))
            ...
            # ob_sequences = np.array()
            # acts_and_advs_seq = np.array()
            # returns_seq = np.array()
            # seq_length = 10
            # for step in range(len(observations)-1-seq_length):
            #     ob_sequences.append(observations[step:step+seq_length])
            #     acts_and_advs_seq.append(acts_and_advs[step+seq_length])
            #     returns_seq.append(returns[step+seq_length])
            # print(observations.shape)

            # Split batches by trial runs
            obs_seqs = utils.split_by_run(observations, dones)
            acts_and_advs_seqs = utils.split_by_run(acts_and_advs, dones)
            returns_seqs = utils.split_by_run(returns, dones)

            # Pad sequences and return as numpy arrays
            obs_seqs = utils.zero_pad_sequence_list(obs_seqs)
            acts_and_advs_seqs = utils.zero_pad_sequence_list(acts_and_advs_seqs)
            returns_seqs = utils.zero_pad_sequence_list(returns_seqs)

            # observations = np.tile(observations,(3,1,1,1,1))
            # acts_and_advs = np.tile(acts_and_advs, (3,1,1))
            # returns = np.tile(returns, (3,1))
            ...
            if(random_action):
                for _ in range(10):
                    losses = self.model.fit(observations, [acts_and_advs, returns], shuffle=False)
                    print(losses)
            else:
                losses = self.model.fit(observations, [acts_and_advs, returns], shuffle=False)
                print(losses)
